...
/**
 * Checks the one invariant the decimal credit design rests on:
 *
 *     COALESCE(<whole>, 0) = FLOOR(<precise>)
 *
 * for every projected column pair, on every row.
 *
...
/**
 * Declared here rather than imported from `credit-precision.ts` on purpose.
 *
 * This script exists to catch the case where the application's idea of the
 * schema is wrong. Sharing the registry would mean a pair dropped from the
 * application also silently drops out of the check, which is exactly when the
 * check is most needed. The duplication is the point; the column-inventory test
 * in `decimal-semantics.itest.ts` keeps the two lists honest against the
 * database.
 */
const PAIRS = [
    {
