...
widget 开出的额度受 `CREDIT_LINE_WIDGET_GRANT_MAX`（新增环境变量）封顶。仓库里没有被跟踪的 `.env.example`，所以新变量记在 `README.md` 的环境变量说明里，并同步到部署密钥库。不设上限的话，任何拿到权限位的 widget 都能给自己开无限额度把平台的钱刷走。本期没有管理端接口，需要突破这个上限只能改 env 或直接改库。
